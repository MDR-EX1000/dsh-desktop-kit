 ...
 */
export function installExecutableAtomically(source, target) {
...
export function apply(ctx, config, overrides = {}) {
    ...
    const open = () => {
        ...
        if (server === undefined || connection === undefined)
            return;
        ...
        if (bin === undefined) {
            log.error(`dsh desktop-kit: no shell binary found; keeping the web surface at ${baseUrl} — ` +
